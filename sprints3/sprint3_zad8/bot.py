...
TOKEN = '6256326280:AAEWyW3L9xlPar3CmihDXGwhk_ANTNOIZBM'  # Замените на ваш токен
...
def get_options_keyboard():
    ...
    resize_btn = types.InlineKeyboardButton("Resize for Sticker", callback_data="resize_for_sticker")
    joke_btn = types.InlineKeyboardButton("Joke!", callback_data="random_joke")
    compliment_bth = types.InlineKeyboardButton("Compliment!", callback_data="random_compliment")
    monet_bth = types.InlineKeyboardButton("Monet!", callback_data="random_monet")
    keyboard.add(pixelate_btn, ascii_btn, invert_btn, reflect_btn, heatmap_btn, resize_btn, joke_btn,
                 compliment_bth, monet_bth)
    return keyboard
...
#       print(compliment)
        ...
        bot.send_message(call.message.chat.id, compliment)  # Отправляем комплимент пользователю
    elif call.data == "random_monet":
        side = random.choice(MONET)  # Выбираем орел или решка
#       print(side)
        bot.answer_callback_query(call.id, "ВЫПАЛО:")
        bot.send_message(call.message.chat.id, side)  # Отправляем комплимент пользователю
