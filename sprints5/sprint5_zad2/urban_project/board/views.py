...
def edit_advertisement(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            return redirect('board:advertisement_list')  # Перенаправление после успешного редактирования
    else:
...
def delete_advertisement(request, pk):
    ...
    if request.method == "POST":
        advertisement.delete()  # Удаляем объявление
        return redirect('board:advertisement_list')  # Перенаправляем на страницу списка объявлений
