...
Реализовано: tk.OptionMenu для создания выпадающего списка. Заполнен предопределенными значениями, sizes = [1, 2, 5, 7, 10]. При выборе размера из списка обновляется текущий размер кисти
