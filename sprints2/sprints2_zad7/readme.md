...
5.Добавление параметра для предварительного просмотра цвета кисти.
Добавлено: Label для цветового предварительного просмотра – Теперь в интерфейсе есть label,
который показывает текущий цвет кисти. Обновление цвета – Метод update_color_preview()
обновляет цвет фона этого label как при выборе цвета через диалог, так и при использовании "пипетки".

6.Добавление возможности изменения размера холста.Реализовано следующее: Для изменения размера холста добавили кнопку,
которая открывает диалоговое окно (tk.simpledialog.askinteger) для ввода новых размеров. После получения новых размеров
обновляем свойства width и height холста и создаем новый объект Image с новыми размерами...

7.Добавление инструмента "Текст" для добавления  текста на изображение. 
Реализован инструмент "Текст", добавлена кнопка "Текст", открывающая диалоговое окно (tk.simpledialog.askstring)
для ввода текста пользователем. После ввода текста и выбора места на холсте кликом левой кнопки мыши используем
ImageDraw.text((x, y), text, fill=self.pen_color) для добавления текста на изображение.

Добавлена функция изменения цвета фона холста.
Добавлена в интерфейс кнопка "Изменить фон", которая вызывает диалоговое окно для выбора цвета
(colorchooser.askcolor()). В обработчике этой кнопки получаем выбранный пользователем цвет и 
применяем его к фону холста, используя метод self.canvas.config(background=new_color).
При этом имеющийся рисунок не сохраняем. 